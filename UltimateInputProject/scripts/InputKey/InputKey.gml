function InputKey(key, device) constructor {
	static init = function(key, device) {
		...
		if (self.key < KEY.A) {
			self.origin = "SPECIAL";
		} else if (self.key < KEY.SHIFT_LEFT) {
			self.origin = "KEYBOARD";
		} else if (self.key < KEY.LEFT_CLICK) {
			self.origin = "KEYBOARD_DIRECT";
		} else if (self.key < KEY.WHEEL_UP) {
			self.origin = "MOUSE_BUTTON";
		} else if (self.key < KEY.MOUSE_UP) {
			self.origin = "MOUSE_WHEEL";
		} else if (self.key < KEY.FACE1) {
			self.origin = "MOUSE_POSITION";
		} else if (self.key < KEY.LEFT_TRIGGER) {
			self.origin = "GAMEPAD_BUTTON";
		} else if (self.key < KEY.LEFT_JOYSTICK_DOWN) {
			self.origin = "TRIGGER";
		} else if (self.key < KEY.LEFT_JOYSTICK_UP) {
			self.origin = "JOYSTICK_POSITIVE";
		} else {
			self.origin = "JOYSTICK_NEGATIVE";
		}
		...
		switch (self.origin) {
			case "SPECIAL":
				self.Check = (self.key == KEY.ANY)
					? InputCallbackSpecialAny
					: InputCallbackSpecialNone;
				break;
			case "KEYBOARD":
			...
			case "MOUSE_BUTTON":
				...
				break;
			case "MOUSE_WHEEL":
				self.Check = (self.key == KEY.WHEEL_UP)
					? InputCallbackMouseWheelUp
					: InputCallbackMouseWheelDown;
				break;
			case "MOUSE_POSITION":
				if (self.key == KEY.MOUSE_UP) {
					self.Check = InputCallbackMouseUp;
				}
				if (self.key == KEY.MOUSE_DOWN) {
					self.Check = InputCallbackMouseDown;
				}
				if (self.key == KEY.MOUSE_LEFT) {
					self.Check = InputCallbackMouseLeft;
				}
				if (self.key == KEY.MOUSE_RIGHT) {
					self.Check = InputCallbackMouseRight;
				}
				break;
			case "GAMEPAD_BUTTON":
	...
	static Save = function(file) {
		file_text_write_string(
			file,
			"k;" + KeyToString(self.key) + ";" + string(self.device) + "\n"
		);
	};
	...
	// SPECIAL
	static InputCallbackDefault = function() {
		return false;
	};

	static InputCallbackSpecialAny = function() {
		return GetCurrentKey() != -1;
	};

	static InputCallbackSpecialNone = function() {
		return GetCurrentKey() == -1;
	};

	// KEYBOARD
	static InputCallbackKeyboardCheck = function() {
		return keyboard_check(self.keyCode);
	};

	// KEYBOARD DIRECT
	static InputCallbackKeyboardCheckDirect = function() {
		return keyboard_check_direct(self.keyCode);
	};

	// MOUSE_BUTTON
	static InputCallbackMouseCheck = function() {
		return mouse_check_button(self.keyCode);
	};

	// MOUSE_WHEEL
	static InputCallbackMouseWheelUp = function() {
		return mouse_wheel_up();
	};

	static InputCallbackMouseWheelDown = function() {
	...
	// GAMEPAD_BUTTON
	static InputCallbackGamepadCheck = function() {
		return gamepad_button_check(self.device, self.keyCode);
	};

	// GAMEPAD_STICK
	static InputCallbackGamepadTrigger = function() {
		return gamepad_button_value(self.device, self.keyCode);
	};
